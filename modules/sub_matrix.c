...
int s21_sub_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
    ...
    int flag = ((A != NULL && B != NULL && result != NULL) ? ((A->matrix != NULL && B->matrix != NULL) ? OK : INCORRECT_MATRIX): INCORRECT_MATRIX);

    if(flag == INCORRECT_MATRIX || A->rows != B->rows || A->columns != B->columns) {
